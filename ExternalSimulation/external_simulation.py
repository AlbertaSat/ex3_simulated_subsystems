...
Basically implements a basic subject-observer pattern for subsystems to subscribe to, 
and a thread for receiving updates from the simulation.

A state mutex is used to ensure updates to the state dictionary are atomic.
Any reads to the state dictionary should only need to check if the mutex is locked, 
wait for it to be unlocked, and then read the state dictionary.
...
class ExternalSimulationProvider:
    ...
    def update(self):
        """Update our internal state from the simulation
...
class ExternalSimulationObserver:
    ...
    def update(self):
        """Update the observer with the latest state from the provider
        This should be implemented by any derived classes
